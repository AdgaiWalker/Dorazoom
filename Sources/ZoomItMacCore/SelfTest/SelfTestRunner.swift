...
public enum SelfTestRunner {
    ...
    private static func testTrimSavePreservesOriginal() throws {
        ...
        let exported = URL(fileURLWithPath: "/tmp/DoraZoom-edit-1234.mp4")
    ...
    /// plain K switches to blackboard. White and black ink remain available
    /// colours, but they do not occupy default keyboard shortcuts.
    private static func testWhiteboardAndBlackboardUseZoomItKeys() throws {
        try expect(DrawingShortcutCommandPolicy.command(for: DrawingShortcut(key: "w")) == .setCanvas(.whiteboard),
                   "Expected W to switch to whiteboard")
        try expect(DrawingShortcutCommandPolicy.command(for: DrawingShortcut(key: "k")) == .setCanvas(.blackboard),
                   "Expected K to switch to blackboard")
        try expect(DrawingShortcutCommandPolicy.command(for: DrawingShortcut(key: "w", modifiers: [.shift])) == nil,
                   "Expected Shift+W not to become a white highlighter shortcut")
        try expect(DrawingShortcutCommandPolicy.command(for: DrawingShortcut(key: "k", modifiers: [.shift])) == nil,
                   "Expected Shift+K not to become a black highlighter shortcut")
        try expect(DrawingShortcutPolicy.zoomItDefault.shortcut(for: .setColor(.white)) == nil,
                   "Expected white ink to have no default keyboard shortcut")
        try expect(DrawingShortcutPolicy.zoomItDefault.shortcut(for: .setColor(.black)) == nil,
                   "Expected black ink to have no default keyboard shortcut")
    }
    ...
    private static func testStandardIconIsRoundedSquareWithMargin() throws {
        ...
        guard let icon = DoraZoomAppIcon.standardIcon(size: size) else {
