...
final class VideoClipEditorController: NSObject, NSWindowDelegate, VideoTimelineViewDelegate {
    ...
    private struct ClipSegment {
        ...
        func replacingRange(_ range: CMTimeRange) -> ClipSegment {
            ClipSegment(
                asset: asset,
                range: range,
        ...
        static func load(asset: AVURLAsset) async -> ClipSegment {
            ...
            if let videoTrack {
                ...
                displaySize = CGSize(width: abs(transformedSize.width), height: abs(transformedSize.height))
            } else {
                displaySize = nil
            }
            return ClipSegment(
    ...
    private var suggestedName = "DoraZoom.mov"
    ...
    func present(tempURL: URL, suggestedName: String,
        ...
        Task { @MainActor [weak self] in
            ...
            let segment = await ClipSegment.load(asset: asset)
            segments = [segment]
            joinTransitions = []
            joinKinds = []
    ...
    private func deleteRange(start: Double, end: Double) {
        ...
        for (index, segment) in segments.enumerated() {
            ...
            if leftEnd > segmentStart {
                ...
                pieces.append(Piece(
                    segment: segment.replacingRange(CMTimeRange(start: segment.range.start, duration: cmTime(keptDuration))),
                    index: index,
            ...
            if rightStart < segmentEnd {
                ...
                pieces.append(Piece(
                    segment: segment.replacingRange(CMTimeRange(
                        start: CMTimeAdd(segment.range.start, cmTime(sourceOffset)),
                        duration: cmTime(keptDuration)
                    )),
                    index: index,
                    oldStart: rightStart,
                    oldEnd: segmentEnd,
    ...
    private func buildComposition() -> Built {
        ...
        for (index, segment) in self.segments.enumerated() {
            if let v = segment.videoTrack {
                try? vTrack.insertTimeRange(segment.range, of: v, at: cursor)
                let size = segment.displaySize ?? renderSize
    ...
    private func export(completion: @escaping (URL?) -> Void) {
        ...
        let out = FileManager.default.temporaryDirectory.appendingPathComponent(RecordingFileNaming.temporaryEditFilename(profile: outputProfile))
        guard let session = AVAssetExportSession(asset: built.composition, presetName: AVAssetExportPresetHighestQuality) else {
            completion(nil); return
        }
        session.outputURL = out
        session.outputFileType = outputProfile.avFileType
