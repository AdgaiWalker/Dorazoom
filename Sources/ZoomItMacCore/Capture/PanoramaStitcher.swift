...
private struct SendableUnsafeBuffer<Element>: @unchecked Sendable {
    ...
    func copy(to destination: UnsafeMutablePointer<Element>, from sourceIndex: Int, count: Int) {
        destination.update(from: buffer.baseAddress!.advanced(by: sourceIndex), count: count)
    }
}
...
private struct SendableUnsafeMutableBuffer<Element>: @unchecked Sendable {
    ...
    func pointer(advancedBy index: Int) -> UnsafeMutablePointer<Element> {
...
public enum PanoramaStitcher {
    ...
    static func axisScan(prev: [UInt8], cur: [UInt8], w: Int, h: Int, ignoreTopRows: Int,
        ...
        let previousGradient = prevGradient
        let currentGradient = curGradient
        ...
        let score: @Sendable (_ dx: Int, _ dy: Int, _ useGradientMask: Bool) -> Double? = { dx, dy, useGradientMask in
            ...
            prev.withUnsafeBufferPointer { pp in
                cur.withUnsafeBufferPointer { pc in
                    previousGradient.withUnsafeBufferPointer { pg in
                        currentGradient.withUnsafeBufferPointer { cg in
                            var y = y0
                            while y < y1 {
                                let curRow = y * w
                                let prevRow = (y + dy) * w
        ...
        func bestVertical(useGradientMask: Bool, center: Int? = nil, radius: Int? = nil) -> (shift: Int, score: Double)? {
            ...
            scores.withUnsafeMutableBufferPointer { buf in
                let sendableScores = SendableUnsafeMutableBuffer(buffer: buf)
                DispatchQueue.concurrentPerform(iterations: span) { i in
                    let dy = bounds.lo + i
                    if dy == 0 { return }
                    if let candidate = score(0, dy, useGradientMask) {
                        sendableScores[i] = candidate
                    }
                }
            }
            var bestShift = 0
        ...
        func bestHorizontal(useGradientMask: Bool, center: Int? = nil, radius: Int? = nil) -> (shift: Int, score: Double)? {
            ...
            scores.withUnsafeMutableBufferPointer { buf in
                ...
                DispatchQueue.concurrentPerform(iterations: span) { i in
                    let dx = bounds.lo + i
                    if dx == 0 { return }
                    if let candidate = score(dx, 0, useGradientMask) {
                        sendableScores[i] = candidate
                    }
                }
            }
    ...
    public static func stitch(frames: [Frame], progress: ((Int) -> Void)? = nil, isCancelled: (() -> Bool)? = nil) -> Frame? {
        ...
        let startIndex = frames.count > 12 ? 1 : 0
        ...
        src.withUnsafeBufferPointer { ps in
            dst.withUnsafeMutableBufferPointer { pd in
                written.withUnsafeMutableBufferPointer { pw in
                    let source = SendableUnsafeBuffer(buffer: ps)
                    let destination = SendableUnsafeMutableBuffer(buffer: pd)
                    let writtenPixels = SendableUnsafeMutableBuffer(buffer: pw)
                    DispatchQueue.concurrentPerform(iterations: srcH) { y in
                        ...
                        if axis == .vertical {
                            if originX == 0 && srcW == dstW {
                                if !writtenPixels[dstRowPx] {
                                    source.copy(to: destination.pointer(advancedBy: dstRowPx * 4), from: srcRow, count: srcW * 4)
                                    for px in dstRowPx..<(dstRowPx + srcW) { writtenPixels[px] = true }
                                }
                                return
                            }
                            for x in 0..<srcW {
                                ...
                                let wi = dstRowPx + dx
                                if writtenPixels[wi] { continue }
                                let si = srcRow + x * 4, di = wi * 4
                                destination[di] = source[si]; destination[di + 1] = source[si + 1]
                                destination[di + 2] = source[si + 2]; destination[di + 3] = 255
                                writtenPixels[wi] = true
                            }
                            return
                        }
                        for x in 0..<srcW {
                            ...
                            if weight >= 1 {
                                ...
                                destination[di] = UInt8(Double(source[si]) * nw + Double(destination[di]) * ow)
                                destination[di + 1] = UInt8(Double(source[si + 1]) * nw + Double(destination[di + 1]) * ow)
                                destination[di + 2] = UInt8(Double(source[si + 2]) * nw + Double(destination[di + 2]) * ow)
