...
#     daily-use build, which keeps DoraZoom's canonical bundle id so its TCC
#     grant is stable across updates.
...
if [[ "$SIGN_IDENTITY" == "-" ]]; then
    ...
    DISPLAY_NAME="${ZOOMIT_DISPLAY_NAME:-DoraZoom (Dev)}"
    SIGN_DESC="ad-hoc"
else
...
if [[ ! "$BUNDLE_ID" =~ $BUNDLE_ID_PATTERN ]]; then
    echo "error: ZOOMIT_BUNDLE_ID must be a reverse-DNS bundle identifier such as com.duola.dorazoom (got '$BUNDLE_ID')." >&2
    exit 2
fi
if [[ "$DISPLAY_NAME" == *"<"* || "$DISPLAY_NAME" == *">"* || "$DISPLAY_NAME" == *"&"* ]]; then
    echo "error: ZOOMIT_DISPLAY_NAME must not contain XML control characters '<', '>' or '&'." >&2
    exit 2
fi

# Keep the .app bundle file name stable and plain for local acceptance while
# the display name remains free to include UI punctuation such as "(Dev)".
...
if [[ "$SIGN_IDENTITY" == "-" ]]; then
    DEFAULT_APP_NAME="DoraZoom Dev.app"
else
    DEFAULT_APP_NAME="DoraZoom.app"
fi
APP_NAME="${ZOOMIT_APP_NAME:-$DEFAULT_APP_NAME}"
if [[ "$APP_NAME" != *.app || "$APP_NAME" == */* || "$APP_NAME" == *":"* ]]; then
    echo "error: ZOOMIT_APP_NAME must be a plain app bundle filename ending in .app, such as DoraZoom.app (got '$APP_NAME')." >&2
    exit 2
fi
...
# Architectures. DoraZoom's personal daily build defaults to arm64 to keep the
# app small on the target Apple Silicon Mac. Debug builds stay native for speed.
# Override with ZOOMIT_ARCHS (space-separated, e.g. "arm64 x86_64") only when a
# second architecture is explicitly required; set it to empty to use the
# toolchain default.
...
cp "$BIN_DIR/ZoomIt" "$APP_PATH/Contents/MacOS/$APP_EXECUTABLE_NAME"
...
if [[ -f "$ICON_SOURCE" ]] && command -v sips >/dev/null && command -v iconutil >/dev/null; then
    ICONSET="$(mktemp -d)/DoraZoom.iconset"
    mkdir -p "$ICONSET"
    ...
    iconutil -c icns "$ICONSET" -o "$APP_PATH/Contents/Resources/$APP_ICON_FILE_NAME.icns"
fi
...
<dict>
    ...
    <string>$APP_ICON_FILE_NAME</string>
...
if [[ "$SIGN_IDENTITY" == "-" ]]; then
    ...
    # daily-use app so the two never fight over one TCC record.
    # Entitlements are embedded even for ad-hoc builds so local
    # hardened-runtime tests can still reach the camera and microphone.
...
echo "  architectures: $(lipo -archs "$APP_PATH/Contents/MacOS/$APP_EXECUTABLE_NAME" 2>/dev/null || echo unknown)" >&2
